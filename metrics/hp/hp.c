...
// Calcular cantidad de categorias unicas de pizza
...
// Ingresa los nombres de las categorias a los struct
void categories_fill(Order *orders, int orders_size, int categories_size, Category *categories){
    ...
    for (int i = 0; i < orders_size; i++) {
        ...
        // Si la categoría es nueva agregarla al struct y agregarla a explored
        if (is_explored == 0) {
            strcpy(explored[index], orders[i].pizza_category);
            index++;
        }
...
// Ingresa cantidad de pizzas por categoria a struct
